// admin/sw.js — registered as a classic (non-module) script for
...
// app shell so the admin app can launch offline; never intercepts
...
// delete the marker app's cache (and vice versa), and let its fetch
...
const CACHE_NAME = "khtpc-admin-shell-v2";
...
self.addEventListener("activate", (event) => {
  ...
  // start with "khtpc-admin-shell-" but aren't the current one) —
  // never touch any other cache name, so this worker can never delete
  // the marker app's cache.
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((key) => key.startsWith("khtpc-admin-shell-") && key !== CACHE_NAME)
  ...
  event.respondWith(
    caches.open(CACHE_NAME).then((cache) =>
      fetch(event.request)
        ...
        // by the marker app's service worker.
